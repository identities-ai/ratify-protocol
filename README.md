...
**Status:** alpha — fixture bytes may change between pre-releases · reference implementation complete · 78 canonical test vectors · cross-language interop proven (Go + TypeScript + Python + Rust + C/C++) · Patent Pending.
...
Pick your language. Each one runs the full 78-fixture conformance suite — the same fixtures every other SDK passes byte-for-byte. If you see "all passing," you've proven cross-language interop **on your own machine.**
...
cargo test
# → test result: ok. 1 passed (loads all 78 fixtures)
```
...
The 78 fixtures in `testvectors/v1/` are the canonical conformance set. **Any implementation in any language that passes all 78 is byte-for-byte interoperable with the reference.** This is the contract. (The directory also contains `cross_sdk_vectors.json` — a separate byte-equivalence corpus used by the NxN cross-SDK matrix; it is not one of the 78 canonical fixtures.)

| Implementation | Language | Conformance (`main`) | Install (latest release) |
|---|---|---|---|
| `github.com/identities-ai/ratify-protocol` | Go | ✅ 78/78 | `go get github.com/identities-ai/ratify-protocol@v1.0.0-alpha.15` |
| `@identities-ai/ratify-protocol` | TypeScript | ✅ 78/78 | `npm install @identities-ai/ratify-protocol` |
| `ratify-protocol` | Python | ✅ 78/78 | `pip install ratify-protocol==1.0.0a15` |
| `ratify-protocol` | Rust | ✅ 78/78 | `cargo add ratify-protocol@1.0.0-alpha.15` |
...
│
├── testvectors/v1/        78 canonical fixtures + cross_sdk_vectors.json
│
├── sdks/
