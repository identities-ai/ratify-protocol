...
**Input-bound enforcement.** The size and count limits above bound parsing and verification work; the depth ceiling alone does not, which is why they exist alongside it. `MAX_PROOF_BUNDLE_BYTES` is enforced by the wire codecs **before** JSON parsing — an oversized payload is rejected without ever being parsed. `MAX_JSON_NESTING_DEPTH` is enforced by the codec **during** parse. Count and length limits are enforced during decode. Violations route to the existing `invalid` status (§5.9) with a descriptive `error_reason` (e.g. `invalid: proof bundle exceeds MAX_PROOF_BUNDLE_BYTES`); no new status is introduced.
