...
The verifier's cryptographic core (chain check, hybrid signature, scope intersection, constraint evaluation) is universal — every conformant SDK implements it identically, and the same 79 fixtures regenerate byte-identical across languages.
...
Ratify is an open protocol. Reference implementations exist in **Go**, **TypeScript**, **Python**, **Rust**, and **C/C++**. The 79 canonical test vectors in the `testvectors/v1/` suite ensure that any conformant implementation is byte-for-byte interoperable. Each SDK also ships a provider-test suite covering the three SPEC §17 hooks.
