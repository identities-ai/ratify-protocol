...
ratify-protocol/
├── SPEC.md              ← the spec
├── testvectors/v1/      ← 78 canonical fixtures
...
- All five SDKs (Go, TS, Python, Rust, C/C++) pass all 78 fixtures (by kind: 61 verify + 2 scope + 5 session-token + 5 transaction-receipt + 2 key-rotation + 1 revocation-list + 1 revocation-push + 1 witness-entry).
...
6. TypeScript `tsc --noEmit` clean, conformance tests pass against all 78 fixtures
7. Python `pip install -e '.[dev]'` cold install succeeds, conformance tests pass against all 78 fixtures
8. Rust `cargo build --all-targets` clean, conformance tests pass against all 78 fixtures
